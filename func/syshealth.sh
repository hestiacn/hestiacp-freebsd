...
function read_kv_config_file() {
    ...
    while IFS= read -r str; do
        echo "$str"
    done < "$HESTIA/conf/defaults/$system.conf"
    unset system
}
...
function syshealth_update_user_config_format() {
    ...
    # Create array of known keys in configuration file
    system="user"
    known_keys="NAME PACKAGE CONTACT CRON_REPORTS MD5 RKEY TWOFA QRCODE PHPCLI ROLE SUSPENDED SUSPENDED_USERS SUSPENDED_WEB SUSPENDED_DNS SUSPENDED_MAIL SUSPENDED_DB SUSPENDED_CRON IP_AVAIL IP_OWNED U_USERS U_DISK U_DISK_DIRS U_DISK_WEB U_DISK_MAIL U_DISK_DB U_BANDWIDTH U_WEB_DOMAINS U_WEB_SSL U_WEB_ALIASES U_DNS_DOMAINS U_DNS_RECORDS U_MAIL_DKIM U_MAIL_ACCOUNTS U_MAIL_DOMAINS U_MAIL_SSL U_DATABASES U_CRON_JOBS U_BACKUPS LANGUAGE THEME NOTIFICATIONS PREF_UI_SORT TIME DATE"
    write_kv_config_file
    unset system
    unset known_keys
...
function syshealth_repair_system_config() {
    ...
    if [ -z "$(check_key_exists 'BACKEND_PORT')" ]; then
        ORIGINAL_PORT=$(sed -n "/listen/{s/.*listen[^0-9]*\([0-9][0-9]*\)[ \t]*ssl\;/\1/p;q}" "$HESTIA/nginx/conf/nginx.conf")
    ...
    if [ -z "$(check_key_exists 'USE_SERVER_SMTP')" ]; then
        echo "[ ! ] Adding missing variable to hestia.conf: USE_SERVER_SMTP ('false')"
    ...
    if [ -z "$(check_key_exists 'POLICY_CSRF_STRICTNESS')" ]; then
        echo "[ ! ] Adding missing variable to hestia.conf: POLICY_CSRF_STRICTNESS ('1')"
...
function syshealth_repair_system_cronjobs() {
    ...
    echo "MAILTO=$email" > /var/cron/tabs/hestiaweb
    echo "CONTENT_TYPE=\"text/plain; charset=utf-8\"" >> /var/cron/tabs/hestiaweb
    echo "*/2 * * * * /usr/local/hestia/bin/v-update-sys-queue restart" >> /var/cron/tabs/hestiaweb
    echo "10 00 * * * /usr/local/hestia/bin/v-update-sys-queue daily" >> /var/cron/tabs/hestiaweb
    echo "15 02 * * * /usr/local/hestia/bin/v-update-sys-queue disk" >> /var/cron/tabs/hestiaweb
    echo "10 00 * * * /usr/local/hestia/bin/v-update-sys-queue traffic" >> /var/cron/tabs/hestiaweb
    echo "30 03 * * * /usr/local/hestia/bin/v-update-sys-queue webstats" >> /var/cron/tabs/hestiaweb
    echo "*/5 * * * * /usr/local/hestia/bin/v-update-sys-queue backup" >> /var/cron/tabs/hestiaweb
    echo "10 05 * * * /usr/local/hestia/bin/v-backup-users" >> /var/cron/tabs/hestiaweb
    echo "20 00 * * * /usr/local/hestia/bin/v-update-user-stats" >> /var/cron/tabs/hestiaweb
    echo "*/5 * * * * /usr/local/hestia/bin/v-update-sys-rrd" >> /var/cron/tabs/hestiaweb
    echo "$min $hour * * * /usr/local/hestia/bin/v-update-letsencrypt-ssl" >> /var/cron/tabs/hestiaweb
    echo "41 4 * * * /usr/local/hestia/bin/v-update-sys-hestia-all" >> /var/cron/tabs/hestiaweb
}
...
function syshealth_adapt_hestia_nginx_listen_ports() {
	physical_nics=$(/sbin/ifconfig -l)
	ipv4_scope_global=""
	ipv6_scope_global=""
	
	for nic in $physical_nics; do
		if [ -z "$ipv4_scope_global" ]; then
			ipv4_scope_global=$(/sbin/ifconfig "$nic" 2>/dev/null | grep -w "inet" | awk '{print $2}')
		fi
		if [ -z "$ipv6_scope_global" ]; then
			ipv6_scope_global=$(/sbin/ifconfig "$nic" 2>/dev/null | grep -w "inet6" | grep -v "fe80" | awk '{print $2}')
		fi
	done
...
syshealth_adapt_nginx_resolver() {
    ...
    if grep -qw "1.0.0.1 8.8.4.4 1.1.1.1 8.8.8.8" "$NGINX_CONF"; then
        ...
        for nameserver in $(grep -is '^nameserver' /etc/resolv.conf | cut -d' ' -f2 | tr '\r\n' ' ' | xargs); do
            if echo "$nameserver" | grep -qE '^([0-9]{1,3}\.){3}[0-9]{1,3}$'; then
                if [ -z "$resolver" ]; then
                    resolver="$nameserver"
                else
