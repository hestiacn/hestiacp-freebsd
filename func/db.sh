...
mysql_dump() {
    err="/tmp/e.mysql"
    mysqldmp="mysqldump"
    if [ -f '/usr/local/bin/mariadb-dump' ] || [ -f '/usr/bin/mariadb-dump' ]; then
        mysqldmp="mariadb-dump"
    fi
    ...
    if [ -z "$host" ] || [ "$host" = 'default' ]; then
        ...
        if [ 0 -lt "$check_row" ]; then
            if [ 1 -eq "$check_row" ]; then
                ...
                for db in $host_str; do
                    parse_object_kv_list "$db"
                    weight=$((U_DB_BASES * 100 / MAX_DB)) 2>/dev/null
...
get_mysql_disk_usage() {
    ...
    if [ "$usage" = '' ] || [ "$usage" = 'NULL' ] || [ "${usage%%.*}" -eq 0 ]; then
        usage=1
    fi
    export LC_ALL=C
    usage=$(printf "%0.f\n" "$usage")
