...
is_ip_rdns_valid() {
    ...
    if [ -z "$rdns" ]; then
        rdns=$(dig +short -x "$ip" | head -n 1 | sed 's/\.$//')
    fi

    if [ -n "$rdns" ]; then
        if echo "$rdns" | grep -q "$awk_ip" || echo "$rdns" | grep -q "$rev_awk_ip"; then
            echo $rdns
            return 0
        fi
    fi
...
get_broadcast() {
    ...
    echo "$((I1 | (255 ^ N1))).$((I2 | (255 ^ N2))).$((I3 | (255 ^ N3))).$((I4 | (255 ^ N4)))"
}
