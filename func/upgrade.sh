...
is_debug_build() {
    case "$new_version" in
        *main*|*release*) DEBUG_MODE="true" ;;
    esac

    # Remove pre-release designation tags from display version
    DISPLAY_VER=$(echo "$new_version" | sed 's|-main||g' | sed 's|-release||g')
}
...
upgrade_welcome_message_log() {
	...
	case "$new_version" in
		*main*) echo "BUILD TYPE:            Development snapshot" ;;
		*release*) echo "BUILD TYPE:            Beta release" ;;
...
upgrade_get_version() {
    # Retrieve new version number for Hestia Control Panel from .pkg package
    new_version=$(pkg info hestia | grep Version | awk '{print $3}')
}
...
upgrade_init_backup() {
    ...
    if [ -n "$DB_SYSTEM" ]; then
        case "$DB_SYSTEM" in
            *mysql*) mkdir -p "$HESTIA_BACKUP/conf/mysql/" ;;
            *pgsql*) mkdir -p "$HESTIA_BACKUP/conf/pgsql/" ;;
        esac
    fi
    ...
    if [ -d "/usr/local/etc/roundcube" ]; then
        mkdir -p "$HESTIA_BACKUP/conf/roundcube/"
    fi
    if [ -d "/usr/local/www/snappymail" ]; then
        mkdir -p "$HESTIA_BACKUP/conf/snappymail/"
    fi
    if [ -d "/usr/local/etc/phpmyadmin" ]; then
        mkdir -p "$HESTIA_BACKUP/conf/phpmyadmin/"
    fi
    if [ -d "/usr/local/etc/phppgadmin" ]; then
...
upgrade_start_backup() {
    ...
    if [ -n "$WEB_SYSTEM" ]; then
        ...
        cp -fr "/usr/local/etc/$WEB_SYSTEM/"* "$HESTIA_BACKUP/conf/$WEB_SYSTEM/"
    fi
    if [ -n "$PROXY_SYSTEM" ]; then
        if [ "$DEBUG_MODE" = "true" ]; then
            echo "      ---- $PROXY_SYSTEM"
        fi
        cp -fr "/usr/local/etc/$PROXY_SYSTEM/"* "$HESTIA_BACKUP/conf/$PROXY_SYSTEM/"
    fi
    if [ -n "$IMAP_SYSTEM" ]; then
        if [ "$DEBUG_MODE" = "true" ]; then
            echo "      ---- $IMAP_SYSTEM"
        fi
        cp -fr "/usr/local/etc/$IMAP_SYSTEM/"* "$HESTIA_BACKUP/conf/$IMAP_SYSTEM/"
    fi
    if [ -n "$MAIL_SYSTEM" ]; then
        if [ "$DEBUG_MODE" = "true" ]; then
            echo "      ---- $MAIL_SYSTEM"
        fi
        cp -fr "/usr/local/etc/$MAIL_SYSTEM/"* "$HESTIA_BACKUP/conf/$MAIL_SYSTEM/"
    fi
    if [ -n "$DNS_SYSTEM" ]; then
        if [ "$DNS_SYSTEM" = "bind9" ] || [ "$DNS_SYSTEM" = "named" ]; then
            if [ "$DEBUG_MODE" = "true" ]; then
                echo "      ---- $DNS_SYSTEM"
            fi
            cp -fr "/usr/local/etc/namedb/"* "$HESTIA_BACKUP/conf/$DNS_SYSTEM/"
        fi
    fi
    if [ -n "$DB_SYSTEM" ]; then
        case "$DB_SYSTEM" in
            *mysql*)
                ...
                cp -fr "/usr/local/etc/mysql/"* "$HESTIA_BACKUP/conf/mysql/"
                ;;
            *pgsql*)
                if [ "$DEBUG_MODE" = "true" ]; then
                    echo "      ---- pgsql"
                fi
                cp -fr "/usr/local/etc/postgresql/"* "$HESTIA_BACKUP/conf/pgsql/"
                ;;
        esac
    fi
    if [ -n "$FTP_SYSTEM" ]; then
        ...
        if [ "$FTP_SYSTEM" = "vsftpd" ]; then
            cp -f "/usr/local/etc/$FTP_SYSTEM.conf" "$HESTIA_BACKUP/conf/$FTP_SYSTEM/"
        fi
        if [ "$FTP_SYSTEM" = "proftpd" ]; then
            cp -f "/usr/local/etc/proftpd/proftpd.conf" "$HESTIA_BACKUP/conf/$FTP_SYSTEM/"
        fi
    fi
    if [ -n "$FIREWALL_SYSTEM" ]; then
        if [ "$DEBUG_MODE" = "true" ]; then
        ...
        [ -f "/etc/pf.conf" ] && cp -f /etc/pf.conf "$HESTIA_BACKUP/conf/$FIREWALL_SYSTEM/"
    fi
    if [ -n "$FIREWALL_EXTENSION" ]; then
        if [ "$DEBUG_MODE" = "true" ]; then
            echo "      ---- $FIREWALL_EXTENSION"
        fi
        cp -f "/usr/local/etc/$FIREWALL_EXTENSION/"*.conf "$HESTIA_BACKUP/conf/$FIREWALL_EXTENSION/"
        cp -f "/usr/local/etc/$FIREWALL_EXTENSION/"*.local "$HESTIA_BACKUP/conf/$FIREWALL_EXTENSION/"
    fi
    if [ -f "/etc/ssh/sshd_config" ]; then
        if [ "$DEBUG_MODE" = "true" ]; then
            echo "      ---- sshd"
    ...
    if [ -d "/usr/local/etc/roundcube" ]; then
        ...
        cp -fr /usr/local/etc/roundcube/* "$HESTIA_BACKUP/conf/roundcube"
    fi
    if [ -d "/usr/local/www/snappymail" ]; then
        if [ "$DEBUG_MODE" = "true" ]; then
            echo "      ---- SnappyMail"
        fi
        cp -fr /usr/local/www/snappymail/* "$HESTIA_BACKUP/conf/snappymail"
    fi
    if [ -d "/usr/local/etc/phpmyadmin" ]; then
        if [ "$DEBUG_MODE" = "true" ]; then
            echo "      ---- phpMyAdmin"
        fi
        cp -fr /usr/local/etc/phpmyadmin/* "$HESTIA_BACKUP/conf/phpmyadmin"
    fi
    if [ -d "/usr/local/etc/phppgadmin" ]; then
        if [ "$DEBUG_MODE" = "true" ]; then
            echo "      ---- phppgadmin"
        fi
        cp -fr /usr/local/etc/phppgadmin/* "$HESTIA_BACKUP/conf/phppgadmin"
    fi
}
...
upgrade_start_routine() {
    ...
    check_version() { echo "$@" | awk -F. '{ printf("%d%03d%03d%03d\n", $1,$2,$3,$4); }'; }
    ...
    upgrade_steps=$(ls -v "$HESTIA/install/upgrade/versions/"*.sh 2>/dev/null)
    available_versions=""
    for script in $upgrade_steps; do
        version=$(basename "$script" .sh)
    ...
    if [ "$(check_version "$latest_version")" -gt "$(check_version "$VERSION")" ]; then
        for version_step in $available_versions; do
            if [ "$(check_version "$VERSION")" -lt "$(check_version "$version_step")" ]; then
                upgrade_step_message
                . "$HESTIA/install/upgrade/versions/$version_step.sh"
            fi
        done
        ...
        if [ -e "$HESTIA/install/upgrade/versions/$VERSION.sh" ]; then
            . "$HESTIA/install/upgrade/versions/$VERSION.sh"
        fi
        VERSION="$new_version"
        upgrade_set_version "$VERSION"
        upgrade_refresh_config
...
upgrade_cloudflare_ip() {
    if [ "$WEB_SYSTEM" = "nginx" ] || [ "$PROXY_SYSTEM" = "nginx" ]; then
        cf_ips="$(fetch -q -T5 -w2 -o - https://api.cloudflare.com/client/v4/ips)"
...
upgrade_phppgadmin() {
    if [ -n "$(echo "$DB_SYSTEM" | grep -w 'pgsql')" ]; then
        pga_release=$(cat /usr/local/www/phppgadmin/libraries/lib.inc.php 2>/dev/null | grep appVersion | head -n1 | cut -f2 -d\' | cut -f1 -d-)
        if version_ge "$pga_release" "$pga_v"; then
            ...
            [ -d /usr/local/www/phppgadmin ] || mkdir -p /usr/local/www/phppgadmin
            # Download latest phpMyAdmin release
            fetch -o "/tmp/phppgadmin-v$pga_v.tar.gz" "https://github.com/hestiacp/phppgadmin/releases/download/v$pga_v/phppgadmin-v$pga_v.tar.gz"
            tar xzf "/tmp/phppgadmin-v$pga_v.tar.gz" -C /usr/local/www/phppgadmin/

            if ! version_ge "$pga_release" "7.14.0"; then
                cp -f "$HESTIA_INSTALL_DIR/pga/config.inc.php" /usr/local/etc/phppgadmin/
            fi
            if [ ! -L /usr/local/www/phppgadmin/conf/config.inc.php ]; then
                ln -s /usr/local/etc/phppgadmin/config.inc.php /usr/local/www/phppgadmin/conf
            fi

            rm -f "/tmp/phppgadmin-v$pga_v.tar.gz"
        fi
    fi
}
...
upgrade_phpmyadmin() {
    ...
    if [ -n "$(echo "$DB_SYSTEM" | grep -w 'mysql')" ]; then
        pma_version=$(jq -r .version /usr/local/www/phpmyadmin/package.json 2>/dev/null)
        if version_ge "$pma_version" "$pma_v"; then
            echo "[ * ] phpMyAdmin is up to date (${pma_version})..."
            # Update permissions
            if [ -e /var/run/phpmyadmin/blowfish_secret.inc.php ]; then
                chown root:hestiamail /var/run/phpmyadmin/blowfish_secret.inc.php
                chmod 0640 /var/run/phpmyadmin/blowfish_secret.inc.php
            fi
            chown hestiamail:hestiamail /usr/local/www/phpmyadmin/tmp
            chown -R root:hestiamail /usr/local/etc/phpmyadmin/

            chmod 0770 /usr/local/www/phpmyadmin/tmp
        else
            # Display upgrade information
            echo "[ * ] Upgrading phpMyAdmin to version $pma_v..."
            [ -d /usr/local/www/phpmyadmin ] || mkdir -p /usr/local/www/phpmyadmin

            # Download latest phpMyAdmin release
            fetch -o "/tmp/phpMyAdmin-$pma_v-all-languages.tar.gz" "https://files.phpmyadmin.net/phpMyAdmin/$pma_v/phpMyAdmin-$pma_v-all-languages.tar.gz"

            # Unpack files
            tar xzf "/tmp/phpMyAdmin-$pma_v-all-languages.tar.gz" -C /tmp

            # Delete file to prevent error
            rm -rf /usr/local/www/phpmyadmin/doc/html

            # Overwrite old files
            cp -rf /tmp/phpMyAdmin-$pma_v-all-languages/* /usr/local/www/phpmyadmin
            ...
            if [ ! -d /usr/local/www/phpmyadmin/tmp ]; then
            ...
            rm -rf "/tmp/phpMyAdmin-$pma_v-all-languages"
            rm -f "/tmp/phpMyAdmin-$pma_v-all-languages.tar.gz"
        fi
    fi
}

upgrade_filemanager() {
    FILE_MANAGER_CHECK=$(grep "FILE_MANAGER='false'" "$HESTIA/conf/hestia.conf")
...
upgrade_roundcube() {
    if [ -n "$(echo "$WEBMAIL_SYSTEM" | grep -w 'roundcube')" ]; then
		if [ -d "/usr/local/www/roundcube" ]; then
            ...
            rc_version=$(cat /usr/local/www/roundcube/index.php | grep -o -E '[0-9].[0-9].[0-9]+' | head -1)
...
upgrade_snappymail() {
    if [ -n "$(echo "$WEBMAIL_SYSTEM" | grep -w 'snappymail')" ]; then
        sm_version=$(cat /usr/local/www/snappymail/data/VERSION)
