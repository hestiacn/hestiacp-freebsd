...
rebuild_user_conf() {
    ...
    pw useradd "$user" -s "$shell" -c "$CONTACT" -m -d "$HOMEDIR/$user" > /dev/null 2>&1
    ...
    if [ "$user" = "$ROOT_USER" ]; then
        ...
        pw usermod "$user" -G "hestia-users"
    ...
    # Update password
    chmod u+w /etc/master.passwd
    sed -i "" "s|^$user:[^:]*:|$user:$MD5:|" /etc/master.passwd
    chmod u-w /etc/master.passwd
    pwd_mkdb -p /etc/master.passwd

    # Building directory tree
    if [ -e "$HOMEDIR/$user/conf" ]; then
        chflags -R noschg $HOMEDIR/$user/conf > /dev/null 2>&1
    fi
    ...
    if [ -n "$WEB_SYSTEM" ] && [ "$WEB_SYSTEM" != 'no' ]; then
        ...
        chown -h "$root":"$user" $HOMEDIR/$user/web
    ...
    # Set immutable flag
    chflags schg $HOMEDIR/$user/conf > /dev/null 2>&1
}
...
rebuild_web_domain_conf() {
    ...
    if [ ! -d "$path/$WEB_SYSTEM/conf.d/domains" ]; then
        mkdir -p "$path/$WEB_SYSTEM/conf.d/domains"
    fi
    if [ ! -d "$path/$PROXY_SYSTEM/conf.d/domains" ]; then
        mkdir -p "$path/$PROXY_SYSTEM/conf.d/domains"
    fi
    ...
    if [ -f "$path/$WEB_SYSTEM/conf.d/$domain.conf" ]; then
        rm -f "$path/$WEB_SYSTEM/conf.d/$domain".*.conf
    fi
    if [ -n "$PROXY_SYSTEM" ] && [ -f "$path/$PROXY_SYSTEM/conf.d/$domain.conf" ]; then
        rm -f "$path/$PROXY_SYSTEM/conf.d/$domain".*.conf
    fi
    ...
    chown -h "$user":"$user" "$HOMEDIR/$user/web/$domain"
    ...
    if [ -n "$STATS" ]; then
        ...
        if [ "$STATS" = 'awstats' ]; then
            if [ -e "$path/awstats/$STATS.$domain_idn.conf" ]; then
                rm -f "$path/awstats/$STATS.$domain_idn.conf"
            fi
            ln -f -s "$HOMEDIR/$user/conf/web/$domain/$STATS.conf" "$path/awstats/$STATS.$domain_idn.conf"
        fi
    ...
    for ftp_user in ${FTP_USER//:/ }; do
        if [ -z "$(grep "^$ftp_user:" /etc/master.passwd)" ]; then
            ...
            chmod u+w /etc/master.passwd
            sed -i "" "s|^$ftp_user:[^:]*:|$ftp_user:$ftp_md5:|" /etc/master.passwd
            chmod u-w /etc/master.passwd
            pwd_mkdb -p /etc/master.passwd
    ...
    for auth_user in ${AUTH_USER//:/ }; do
        ...
        if [ "$WEB_SYSTEM" = "nginx" ] || [ "$PROXY_SYSTEM" = "nginx" ]; then
            ...
            htaccess="$HOMEDIR/$user/conf/web/$domain/apache24.conf_htaccess"
            shtaccess="$HOMEDIR/$user/conf/web/$domain/apache24.ssl.conf_htaccess"
            if [ ! -f "$htaccess" ]; then
    ...
    chown -h "$user":"www" "$HOMEDIR/$user/web/$domain/public_"*"html"
}
...
rebuild_dns_domain_conf() {
    ...
    # Get dns config path
    if [ -f "${path}namedb/named.conf" ]; then
        dns_conf="${path}namedb/named.conf"
    fi

    # Bind config check
    if [ "$SUSPENDED" = 'yes' ]; then
        rm_string=$(grep -n "${path}namedb/$domain.db" "$dns_conf" | cut -d : -f 1)
        if [ -n "$rm_string" ]; then
    ...
    # Load new config
    /usr/local/sbin/rndc reconfig > /dev/null 2>&1
    # Reload config
    /usr/local/sbin/rndc reload "$domain" > /dev/null 2>&1

    if [ "$DNSSEC" = "yes" ]; then
        # Key consists always out of 5 digits when less is used they are "lost"
        key=$(/usr/local/sbin/rndc dnssec -status "$domain_idn" | grep ^key: | cut -f2 -d' ' | awk '{printf "%05.0f\n", $0}')
        
        if [ ! -d "$USER_DATA/keys/" ]; then
            mkdir -p "$USER_DATA/keys/"
        fi
        cp "$path/namedb/working/K$domain_idn.+013+$key".* "$USER_DATA/keys/"
...
rebuild_mail_domain_conf() {
    ...
    case "$MAIL_SYSTEM" in
        *exim*)
            # FreeBSD: exim config path
            rm -f $path/$MAIL_SYSTEM/domains/$domain_idn
            mkdir -p "$HOMEDIR/$user/conf/mail/$domain"
            ln -s "$HOMEDIR/$user/conf/mail/$domain" "$path/$MAIL_SYSTEM/domains/$domain_idn"
            ...
            rm -rf "$HOMEDIR/$user/conf/mail/$domain/limits"
            ...
            if [ "$SUSPENDED" = 'yes' ]; then
                rm -f $path/$MAIL_SYSTEM/domains/$domain_idn
                rm -f $path/dovecot/conf.d/domains/"$domain_idn".conf
            fi
        ...
        case "$MAIL_SYSTEM" in
            *exim*)
                ...
                if [ -n "$RATE_LIMIT" ]; then
                    ...
                    system=$(cat $path/exim/limit.conf)
                    sed -i "" "/^$account@$domain_idn:/ d" "$HOMEDIR/$user/conf/mail/$domain/limits"
                    if [ -n "$system" ]; then
    ...
    case "$MAIL_SYSTEM" in
        *exim*)
            ...
            chmod 771 $path/$MAIL_SYSTEM/domains/$domain_idn
