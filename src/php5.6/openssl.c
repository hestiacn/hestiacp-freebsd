...
#define SSL_load_error_strings() OPENSSL_init_ssl(0, NULL)
...
*/
	...
	time_data = ASN1_STRING_get0_data(timestr);
	time_len = ASN1_STRING_length(timestr);
	...
	if (time_len != strlen((const char*)time_data)) {
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "illegal length in timestamp");
		return (time_t)-1;
	}

	if (time_len < 13) {
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to parse time string %s correctly", time_data);
		return (time_t)-1;
	}

	if (ASN1_STRING_type(timestr) == V_ASN1_GENERALIZEDTIME && time_len < 15) {
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "unable to parse time string %s correctly", time_data);
		return (time_t)-1;
	}

	strbuf = estrdup((char *)time_data);

	memset(&thetime, 0, sizeof(thetime));

	/* we work backwards so that we can use atoi more easily */

	thestr = strbuf + time_len - 3;
...
static int openssl_x509v3_subjectAltName(BIO *bio, X509_EXTENSION *extension)
{
	...
	if (method->it) {
#if OPENSSL_VERSION_NUMBER >= 0x40000000L || defined(LIBRESSL_VERSION_NUMBER)
    names = (GENERAL_NAMES*)(ASN1_item_d2i(NULL, &p, length, (const ASN1_ITEM *)method->it));
#else
    names = (GENERAL_NAMES*)(ASN1_item_d2i(NULL, &p, length, ASN1_ITEM_ptr(method->it)));
#endif
	} else {
...
#if OPENSSL_VERSION_NUMBER >= 0x40000000L || defined(LIBRESSL_VERSION_NUMBER)
    char *name = (char *)X509_alias_get0(cert, NULL);
    if (name) {
...
/*	add_assoc_bool(return_value, "valid", cert->valid); */
	...
	for (i = 0; i < X509_get_ext_count(cert); i++) {
		int nid;
		extension = (X509_EXTENSION *)X509_get_ext(cert, i);
			...
			case OPENSSL_KEYTYPE_RSA:
...
#if OPENSSL_VERSION_NUMBER >= 0x40000000L || defined(LIBRESSL_VERSION_NUMBER)
				{
					...
					if (ctx && EVP_PKEY_keygen_init(ctx) > 0) {
						if (EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, req->priv_key_bits) > 0) {
							if (EVP_PKEY_keygen(ctx, &req->priv_key) > 0) {
...
#else
			if (rsa != NULL && (NULL == rsa->p || NULL == rsa->q)) {
				return 0;
			}
#endif
			break;
...
#else
			if (NULL == dsa->p || NULL == dsa->q || NULL == dsa->priv_key) {
				return 0;
			}
#endif
			break;
		...
		if (zend_hash_find(Z_ARRVAL_P(args), "rsa", sizeof("rsa"), (void**)&data) == SUCCESS &&
		    ...
		    if (pkey) {
				...
				if (rsa) {
					...
					/* 从数组中提取 BIGNUM 值 */
					if (zend_hash_find(Z_ARRVAL_PP(data), "n", sizeof("n"), (void**)&bn) == SUCCESS &&
					    Z_TYPE_PP(bn) == IS_STRING) {
						n = BN_bin2bn((unsigned char*)Z_STRVAL_PP(bn), Z_STRLEN_PP(bn), NULL);
					}
					if (zend_hash_find(Z_ARRVAL_PP(data), "e", sizeof("e"), (void**)&bn) == SUCCESS &&
					    Z_TYPE_PP(bn) == IS_STRING) {
						e = BN_bin2bn((unsigned char*)Z_STRVAL_PP(bn), Z_STRLEN_PP(bn), NULL);
					}
					...
					if (zend_hash_find(Z_ARRVAL_PP(data), "p", sizeof("p"), (void**)&bn) == SUCCESS &&
					    Z_TYPE_PP(bn) == IS_STRING) {
						p = BN_bin2bn((unsigned char*)Z_STRVAL_PP(bn), Z_STRLEN_PP(bn), NULL);
					}
					if (zend_hash_find(Z_ARRVAL_PP(data), "q", sizeof("q"), (void**)&bn) == SUCCESS &&
					    Z_TYPE_PP(bn) == IS_STRING) {
						q = BN_bin2bn((unsigned char*)Z_STRVAL_PP(bn), Z_STRLEN_PP(bn), NULL);
					}
					if (zend_hash_find(Z_ARRVAL_PP(data), "dmp1", sizeof("dmp1"), (void**)&bn) == SUCCESS &&
					    Z_TYPE_PP(bn) == IS_STRING) {
						dmp1 = BN_bin2bn((unsigned char*)Z_STRVAL_PP(bn), Z_STRLEN_PP(bn), NULL);
					}
					if (zend_hash_find(Z_ARRVAL_PP(data), "dmq1", sizeof("dmq1"), (void**)&bn) == SUCCESS &&
					    Z_TYPE_PP(bn) == IS_STRING) {
						dmq1 = BN_bin2bn((unsigned char*)Z_STRVAL_PP(bn), Z_STRLEN_PP(bn), NULL);
					}
					if (zend_hash_find(Z_ARRVAL_PP(data), "iqmp", sizeof("iqmp"), (void**)&bn) == SUCCESS &&
					    Z_TYPE_PP(bn) == IS_STRING) {
						iqmp = BN_bin2bn((unsigned char*)Z_STRVAL_PP(bn), Z_STRLEN_PP(bn), NULL);
					}
		    ...
		    if (pkey) {
				...
				if (dsa) {
					...
					if (zend_hash_find(Z_ARRVAL_PP(data), "g", sizeof("g"), (void**)&bn) == SUCCESS &&
					    Z_TYPE_PP(bn) == IS_STRING) {
						g = BN_bin2bn((unsigned char*)Z_STRVAL_PP(bn), Z_STRLEN_PP(bn), NULL);
					}
					if (zend_hash_find(Z_ARRVAL_PP(data), "priv_key", sizeof("priv_key"), (void**)&bn) == SUCCESS &&
					    Z_TYPE_PP(bn) == IS_STRING) {
						priv_key = BN_bin2bn((unsigned char*)Z_STRVAL_PP(bn), Z_STRLEN_PP(bn), NULL);
					}
					if (zend_hash_find(Z_ARRVAL_PP(data), "pub_key", sizeof("pub_key"), (void**)&bn) == SUCCESS &&
					    Z_TYPE_PP(bn) == IS_STRING) {
						pub_key = BN_bin2bn((unsigned char*)Z_STRVAL_PP(bn), Z_STRLEN_PP(bn), NULL);
					}
		...
		case EVP_PKEY_RSA2:
...
#if OPENSSL_VERSION_NUMBER >= 0x40000000L || defined(LIBRESSL_VERSION_NUMBER)
			{
				...
				if (rsa != NULL) {
					...
					if (n) { char *str = emalloc(BN_num_bytes(n) + 1); BN_bn2bin(n, (unsigned char*)str); str[BN_num_bytes(n)] = 0; add_assoc_stringl(rsa_arr, "n", str, BN_num_bytes(n), 0); }
					if (e) { char *str = emalloc(BN_num_bytes(e) + 1); BN_bn2bin(e, (unsigned char*)str); str[BN_num_bytes(e)] = 0; add_assoc_stringl(rsa_arr, "e", str, BN_num_bytes(e), 0); }
					if (d) { char *str = emalloc(BN_num_bytes(d) + 1); BN_bn2bin(d, (unsigned char*)str); str[BN_num_bytes(d)] = 0; add_assoc_stringl(rsa_arr, "d", str, BN_num_bytes(d), 0); }
					if (p) { char *str = emalloc(BN_num_bytes(p) + 1); BN_bn2bin(p, (unsigned char*)str); str[BN_num_bytes(p)] = 0; add_assoc_stringl(rsa_arr, "p", str, BN_num_bytes(p), 0); }
					if (q) { char *str = emalloc(BN_num_bytes(q) + 1); BN_bn2bin(q, (unsigned char*)str); str[BN_num_bytes(q)] = 0; add_assoc_stringl(rsa_arr, "q", str, BN_num_bytes(q), 0); }
					if (dmp1) { char *str = emalloc(BN_num_bytes(dmp1) + 1); BN_bn2bin(dmp1, (unsigned char*)str); str[BN_num_bytes(dmp1)] = 0; add_assoc_stringl(rsa_arr, "dmp1", str, BN_num_bytes(dmp1), 0); }
					if (dmq1) { char *str = emalloc(BN_num_bytes(dmq1) + 1); BN_bn2bin(dmq1, (unsigned char*)str); str[BN_num_bytes(dmq1)] = 0; add_assoc_stringl(rsa_arr, "dmq1", str, BN_num_bytes(dmq1), 0); }
					if (iqmp) { char *str = emalloc(BN_num_bytes(iqmp) + 1); BN_bn2bin(iqmp, (unsigned char*)str); str[BN_num_bytes(iqmp)] = 0; add_assoc_stringl(rsa_arr, "iqmp", str, BN_num_bytes(iqmp), 0); }
					
					add_assoc_zval(return_value, "rsa", rsa_arr);
				}
			}
#else
			if (pkey->pkey.rsa != NULL) {
				...
				ALLOC_INIT_ZVAL(rsa_arr);
				array_init(rsa_arr);
				OPENSSL_PKEY_GET_BN(rsa_arr, n);
				OPENSSL_PKEY_GET_BN(rsa_arr, e);
				OPENSSL_PKEY_GET_BN(rsa_arr, d);
				OPENSSL_PKEY_GET_BN(rsa_arr, p);
				OPENSSL_PKEY_GET_BN(rsa_arr, q);
				OPENSSL_PKEY_GET_BN(rsa_arr, dmp1);
				OPENSSL_PKEY_GET_BN(rsa_arr, dmq1);
				OPENSSL_PKEY_GET_BN(rsa_arr, iqmp);
				add_assoc_zval(return_value, "rsa", rsa_arr);
			}
#endif
			break;
		...
		case EVP_PKEY_DSA4:
...
#if OPENSSL_VERSION_NUMBER >= 0x40000000L || defined(LIBRESSL_VERSION_NUMBER)
			{
				...
				if (dsa != NULL) {
					...
					ALLOC_INIT_ZVAL(dsa_arr);
					array_init(dsa_arr);
					
					DSA_get0_pqg(dsa, &p, &q, &g);
					DSA_get0_key(dsa, &pub_key, &priv_key);
					
					if (p) { char *str = emalloc(BN_num_bytes(p) + 1); BN_bn2bin(p, (unsigned char*)str); str[BN_num_bytes(p)] = 0; add_assoc_stringl(dsa_arr, "p", str, BN_num_bytes(p), 0); }
					if (q) { char *str = emalloc(BN_num_bytes(q) + 1); BN_bn2bin(q, (unsigned char*)str); str[BN_num_bytes(q)] = 0; add_assoc_stringl(dsa_arr, "q", str, BN_num_bytes(q), 0); }
					if (g) { char *str = emalloc(BN_num_bytes(g) + 1); BN_bn2bin(g, (unsigned char*)str); str[BN_num_bytes(g)] = 0; add_assoc_stringl(dsa_arr, "g", str, BN_num_bytes(g), 0); }
					if (priv_key) { char *str = emalloc(BN_num_bytes(priv_key) + 1); BN_bn2bin(priv_key, (unsigned char*)str); str[BN_num_bytes(priv_key)] = 0; add_assoc_stringl(dsa_arr, "priv_key", str, BN_num_bytes(priv_key), 0); }
					if (pub_key) { char *str = emalloc(BN_num_bytes(pub_key) + 1); BN_bn2bin(pub_key, (unsigned char*)str); str[BN_num_bytes(pub_key)] = 0; add_assoc_stringl(dsa_arr, "pub_key", str, BN_num_bytes(pub_key), 0); }
					
					add_assoc_zval(return_value, "dsa", dsa_arr);
				}
			}
#else
			if (pkey->pkey.dsa != NULL) {
				...
				ALLOC_INIT_ZVAL(dsa_arr);
				array_init(dsa_arr);
				OPENSSL_PKEY_GET_BN(dsa_arr, p);
				OPENSSL_PKEY_GET_BN(dsa_arr, q);
				OPENSSL_PKEY_GET_BN(dsa_arr, g);
				OPENSSL_PKEY_GET_BN(dsa_arr, priv_key);
				OPENSSL_PKEY_GET_BN(dsa_arr, pub_key);
				add_assoc_zval(return_value, "dsa", dsa_arr);
			}
#endif
			break;
		...
		case EVP_PKEY_DH:
...
#if OPENSSL_VERSION_NUMBER >= 0x40000000L || defined(LIBRESSL_VERSION_NUMBER)
			{
				...
				if (dh != NULL) {
					...
					ALLOC_INIT_ZVAL(dh_arr);
					array_init(dh_arr);
					
					DH_get0_pqg(dh, &p, NULL, &g);
					DH_get0_key(dh, &pub_key, &priv_key);
					
					if (p) { char *str = emalloc(BN_num_bytes(p) + 1); BN_bn2bin(p, (unsigned char*)str); str[BN_num_bytes(p)] = 0; add_assoc_stringl(dh_arr, "p", str, BN_num_bytes(p), 0); }
					if (g) { char *str = emalloc(BN_num_bytes(g) + 1); BN_bn2bin(g, (unsigned char*)str); str[BN_num_bytes(g)] = 0; add_assoc_stringl(dh_arr, "g", str, BN_num_bytes(g), 0); }
					if (priv_key) { char *str = emalloc(BN_num_bytes(priv_key) + 1); BN_bn2bin(priv_key, (unsigned char*)str); str[BN_num_bytes(priv_key)] = 0; add_assoc_stringl(dh_arr, "priv_key", str, BN_num_bytes(priv_key), 0); }
					if (pub_key) { char *str = emalloc(BN_num_bytes(pub_key) + 1); BN_bn2bin(pub_key, (unsigned char*)str); str[BN_num_bytes(pub_key)] = 0; add_assoc_stringl(dh_arr, "pub_key", str, BN_num_bytes(pub_key), 0); }
					
					add_assoc_zval(return_value, "dh", dh_arr);
				}
			}
#else
			if (pkey->pkey.dh != NULL) {
				...
				ALLOC_INIT_ZVAL(dh_arr);
				array_init(dh_arr);
				OPENSSL_PKEY_GET_BN(dh_arr, p);
				OPENSSL_PKEY_GET_BN(dh_arr, g);
				OPENSSL_PKEY_GET_BN(dh_arr, priv_key);
				OPENSSL_PKEY_GET_BN(dh_arr, pub_key);
				add_assoc_zval(return_value, "dh", dh_arr);
			}
#endif
			break;
...
#if OPENSSL_VERSION_NUMBER >= 0x40000000L || defined(LIBRESSL_VERSION_NUMBER)
			{
				...
				if (ec != NULL) {
					...
					if (nid != NID_undef) {
						...
						if (crv_sn != NULL) {
							add_assoc_string(ec_arr, "curve_name", crv_sn, 1);
						}

						obj = OBJ_nid2obj(nid);
						if (obj != NULL) {
							int oir_len = OBJ_obj2txt(oir_buf, sizeof(oir_buf), obj, 1);
							add_assoc_stringl(ec_arr, "curve_oid", (char*)oir_buf, oir_len, 1);
							ASN1_OBJECT_free(obj);
						}
					}

					add_assoc_zval(return_value, "ec", ec_arr);
				}
			}
#else
...
PHP_FUNCTION(openssl_sign)
{
	...
	if (!md_ctx) {
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to allocate EVP_MD_CTX");
		efree(sigbuf);
		if (keyresource == -1) {
			EVP_PKEY_free(pkey);
		}
...
#else
	EVP_MD_CTX md_ctx_stack;
	EVP_SignInit(&md_ctx_stack, mdtype);
	EVP_SignUpdate(&md_ctx_stack, data, data_len);
	if (EVP_SignFinal(&md_ctx_stack, sigbuf, (unsigned int *)&siglen, pkey)) {
		zval_dtor(signature);
		sigbuf[siglen] = '\0';
		ZVAL_STRINGL(signature, (char *)sigbuf, siglen, 0);
		RETVAL_TRUE;
	...
	EVP_MD_CTX_cleanup(&md_ctx_stack);
#endif
...
#else
	EVP_MD_CTX md_ctx_stack;
	EVP_VerifyInit(&md_ctx_stack, mdtype);
	EVP_VerifyUpdate(&md_ctx_stack, data, data_len);
	err = EVP_VerifyFinal(&md_ctx_stack, (unsigned char *)signature, signature_len, pkey);
	EVP_MD_CTX_cleanup(&md_ctx_stack);
#endif
...
PHP_FUNCTION(openssl_seal)
{
	...
	if (!ctx) {
		php_error_docref(NULL TSRMLS_CC, E_WARNING, "Failed to allocate EVP_CIPHER_CTX");
		RETVAL_FALSE;
		goto clean_exit;
	}
...
#else
	if (!EVP_EncryptInit(&ctx_stack, cipher, NULL, NULL)) {
		RETVAL_FALSE;
		goto clean_exit;
	}
#endif
	...
	buf = emalloc(data_len + EVP_CIPHER_block_size(cipher) + 1);
...
#else
	EVP_CIPHER_CTX ctx_stack;
	if (EVP_SealInit(&ctx_stack, cipher, eks, eksl, NULL, pkeys, nkeys) <= 0 ||
			!EVP_SealUpdate(&ctx_stack, buf, &len1, (unsigned char *)data, data_len) ||
			!EVP_SealFinal(&ctx_stack, buf + len1, &len2)) {
		RETVAL_FALSE;
		efree(buf);
		goto clean_exit;
	}
...
#else
	EVP_CIPHER_CTX_cleanup(&ctx_stack);
#endif
	for (i=0; i<nkeys; i++) {
...
PHP_FUNCTION(openssl_open)
{
	...
	if (EVP_OpenInit(ctx, cipher, (unsigned char *)ekey, ekey_len, NULL, pkey) && 
...
#else
	EVP_CIPHER_CTX ctx_stack;
	if (EVP_OpenInit(&ctx_stack, cipher, (unsigned char *)ekey, ekey_len, NULL, pkey) && 
	    EVP_OpenUpdate(&ctx_stack, buf, &len1, (unsigned char *)data, data_len)) {
		if (!EVP_OpenFinal(&ctx_stack, buf + len1, &len2) || (len1 + len2 == 0)) {
			efree(buf);
			RETVAL_FALSE;
		} else {
			zval_dtor(opendata);
...
#else
	EVP_MD_CTX md_ctx_stack;
	EVP_DigestInit(&md_ctx_stack, mdtype);
	EVP_DigestUpdate(&md_ctx_stack, (unsigned char *)data, data_len);
	if (EVP_DigestFinal(&md_ctx_stack, (unsigned char *)sigbuf, (unsigned int *)&siglen)) {
		if (raw_output) {
			sigbuf[siglen] = '\0';
			RETVAL_STRINGL((char *)sigbuf, siglen, 0);
		} else {
...
PHP_FUNCTION(openssl_encrypt)
{
...
#else
	EVP_CIPHER_CTX cipher_ctx_stack;
	EVP_EncryptInit(&cipher_ctx_stack, cipher_type, NULL, NULL);
	if (password_len > keylen) {
		EVP_CIPHER_CTX_set_key_length(&cipher_ctx_stack, password_len);
	}
	EVP_EncryptInit_ex(&cipher_ctx_stack, NULL, NULL, key, (unsigned char *)iv);
	if (options & OPENSSL_ZERO_PADDING) {
		EVP_CIPHER_CTX_set_padding(&cipher_ctx_stack, 0);
	}
	if (data_len > 0) {
		EVP_EncryptUpdate(&cipher_ctx_stack, outbuf, &i, (unsigned char *)data, data_len);
	}
	outlen = i;
	if (EVP_EncryptFinal(&cipher_ctx_stack, (unsigned char *)outbuf + i, &i)) {
		outlen += i;
		if (options & OPENSSL_RAW_DATA) {
			outbuf[outlen] = '\0';
			RETVAL_STRINGL_CHECK((char *)outbuf, outlen, 0);
	...
	EVP_CIPHER_CTX_cleanup(&cipher_ctx_stack);
#endif
...
#else
	EVP_CIPHER_CTX cipher_ctx_stack;
	EVP_DecryptInit(&cipher_ctx_stack, cipher_type, NULL, NULL);
	if (password_len > keylen) {
		EVP_CIPHER_CTX_set_key_length(&cipher_ctx_stack, password_len);
	}
	EVP_DecryptInit_ex(&cipher_ctx_stack, NULL, NULL, key, (unsigned char *)iv);
	if (options & OPENSSL_ZERO_PADDING) {
		EVP_CIPHER_CTX_set_padding(&cipher_ctx_stack, 0);
	}
	EVP_DecryptUpdate(&cipher_ctx_stack, outbuf, &i, (unsigned char *)data, data_len);
	outlen = i;
	if (EVP_DecryptFinal(&cipher_ctx_stack, (unsigned char *)outbuf + i, &i)) {
		outlen += i;
		outbuf[outlen] = '\0';
		RETVAL_STRINGL((char *)outbuf, outlen, 0);
	} else {
...
PHP_FUNCTION(openssl_dh_compute_key)
{
	...
	if (!pkey || EVP_PKEY_id(pkey) != EVP_PKEY_DH || !dh) {
		RETURN_FALSE;
	}
