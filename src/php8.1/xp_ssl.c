...
#define EXPAND_IPV6_ADDRESS(_str, _bytes) \
	do { \
		...
		snprintf(_str, 40, "%X:%X:%X:%X:%X:%X:%X:%X", \
			...
			__data[6] << 8 | __data[7], \
			__data[8] << 8 | __data[9], \
			__data[10] << 8 | __data[11], \
			__data[12] << 8 | __data[13], \
			__data[14] << 8 | __data[15] \
		); \
	} while(0)
...
#ifdef HAVE_IPV6_SAN
			else if (ASN1_STRING_length(san->d.iPAddress) == 16 && subject_name_is_ipv6) {
				ipbuffer[0] = 0;
				const unsigned char *ip_data = ASN1_STRING_get0_data(san->d.iPAddress);
