...
# PHP 8.3 includes IMAP support in core (no PECL needed)
...
create_package() {
    ...
    for ext in "${extensions[@]}"; do
        if [ -f "$EXTENSION_DIR/${ext}.so" ]; then
            ...
            echo "⚠️  ${ext}.so not found (PHP 8.3 includes imap in core)"
        fi
    done
...
PHP 8.3 is a stable release with long-term support.
EOD
EOF
