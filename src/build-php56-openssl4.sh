...
get_config_args() {
	...
	local openssl_prefix="${OPENSSL_PREFIX:-/usr/local}"
...
build_php() {
	...
	# ✅ 设置 PKG_CONFIG_PATH
	export PKG_CONFIG_PATH="${OPENSSL_PREFIX}/lib/pkgconfig:${ICU_PREFIX}/lib/pkgconfig:/usr/local/lib/pkgconfig:/usr/local/libdata/pkgconfig:/usr/lib/pkgconfig"
	
	# ✅ 设置编译标志
	export CPPFLAGS="-I${ICU_PREFIX}/include -I${OPENSSL_PREFIX}/include"
	
	export CFLAGS="-I${ICU_PREFIX}/include -I${OPENSSL_PREFIX}/include \
	...
	export LDFLAGS="-L${ICU_PREFIX}/lib -L${OPENSSL_PREFIX}/lib \
		-Wl,-rpath,${ICU_PREFIX}/lib -Wl,-rpath,${OPENSSL_PREFIX}/lib"
	export LIBS="-licui18n -licuuc -licudata -lssl -lcrypto -lc++ -lpq -lintl -lpthread -lm"
	export LD_LIBRARY_PATH="${ICU_PREFIX}/lib:${OPENSSL_PREFIX}/lib:$LD_LIBRARY_PATH"
	...
	# 验证库文件
	echo "[ * ] Verifying ICU 53 libraries:"
	ls -la ${ICU_PREFIX}/lib/libicu*.so.* | head -5
	
	echo "[ * ] Verifying OpenSSL libraries:"
	ls -la ${OPENSSL_PREFIX}/lib/libssl.so.* ${OPENSSL_PREFIX}/lib/libcrypto.so.* | head -5
	
	echo "[ * ] Creating OpenSSL version symlinks..."
	if [ -f /usr/local/lib/libssl.so.30 ] && [ ! -f /usr/local/lib/libssl.so ]; then
		ln -sf /usr/local/lib/libssl.so.30 /usr/local/lib/libssl.so
		echo "[ ✓ ] Created libssl.so -> libssl.so.30"
	fi
	
	if [ -f /usr/local/lib/libcrypto.so.30 ] && [ ! -f /usr/local/lib/libcrypto.so ]; then
		ln -sf /usr/local/lib/libcrypto.so.30 /usr/local/lib/libcrypto.so
		echo "[ ✓ ] Created libcrypto.so -> libcrypto.so.30"
	fi
	...
	if [ -f "Makefile" ]; then
		...
		# 确保 ICU 库在前，OpenSSL 在后
		if ! grep -q "\-licui18n" Makefile; then
			sed -i '' 's/^EXTRA_LIBS = \(.*\)$/EXTRA_LIBS = -licui18n -licuuc -licudata -lssl -lcrypto \1/' Makefile
			echo "[ ✓ ] Added ICU and OpenSSL to EXTRA_LIBS"
		fi
		...
		# 添加 rpath 确保运行时找到正确的库
		if ! grep -q "\-rpath,\/usr\/local\/icu53" Makefile; then
			sed -i '' 's/^LDFLAGS = \(.*\)$/LDFLAGS = \1 -Wl,-rpath,\/usr\/local\/icu53\/lib -Wl,-rpath,\/usr\/local\/lib/' Makefile
			echo "[ ✓ ] Added rpath to LDFLAGS"
		fi
