...
build_icu53() {
    ...
    if [ -f "config/icu-config" ]; then
        ...
        ls -la config/icu-config 2>/dev/null || echo "Not found"
        echo "Checking if icu-config exists:"
        ls -la icu-config 2>/dev/null || echo "Not found"
        return 1
    fi
    ...
    if [ ! -f "$icu_prefix/bin/icu-config" ]; then
        ...
        chmod +x "$icu_prefix/bin/icu-config" 2>/dev/null || true
    fi
    ...
    if [ ! -f "$icu_prefix/lib/libicuuc.so.53.2" ] || \
       ...
       [ ! -f "$icu_prefix/bin/icu-config" ]; then
        ...
        ls -la "$icu_prefix/lib/" 2>/dev/null || echo "Directory not found"
        echo "Files in $icu_prefix/bin:"
        ls -la "$icu_prefix/bin/" 2>/dev/null || echo "Directory not found"
        return 1
    fi
