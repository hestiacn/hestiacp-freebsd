...
build_php() {
    ...
    if [ "$major" = "5" ] && [ "$PHP_VERSION" = "5.6.40" ]; then
        ...
        export LDFLAGS="-L/usr/local/icu53/lib -Wl,-rpath,/usr/local/icu53/lib -licuuc -licui18n -licudata -lc++"
    ...
    export LIBS="-licui18n -licuuc -licudata -lc++ -lpthread -lm"
    ...
    if [ -f "Makefile" ]; then
        ...
        sed -i '' 's/^EXTRA_LIBS = .*/EXTRA_LIBS = -licuuc -licui18n -licudata -lc++ -lpthread -lm/' Makefile
        sed -i '' 's|^LDFLAGS = .*|LDFLAGS = -L/usr/local/icu53/lib -Wl,-rpath,/usr/local/icu53/lib -licuuc -licui18n -licudata -lc++|' Makefile
