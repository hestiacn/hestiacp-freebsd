...
build_icu53() {
    ...
    # ✅ 生成 icu-config
    echo "[ * ] Generating icu-config via config.status..."
    ./config.status --file=icu-config
    
    if [ ! -f "icu-config" ]; then
        echo "❌ Failed to generate icu-config"
        return 1
    fi
