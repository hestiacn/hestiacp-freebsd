...
build_icu53() {
    ...
    # ✅ 检查 icu-config 是否已生成（Makefile 会自动生成）
    ...
    if [ -f "config/icu-config" ]; then
        ...
        echo "Checking if config/icu-config exists:"
        ls -la config/icu-config || echo "Not found"
        echo "Checking if icu-config exists:"
        ls -la icu-config || echo "Not found"
        return 1
    fi
    ...
    # ✅ 修改 icu-config 中的 libdir 检查（如果必要）
    echo "[ * ] Fixing icu-config library path check..."
    if [ -f "$icu_prefix/bin/icu-config" ]; then
        # 让 icu-config 使用 .so.53.2 而不是 .so
    ...
    if ! "$icu_prefix/bin/icu-config" --version > /dev/null 2>&1; then
        ...
        echo "Debug: running icu-config --version"
        "$icu_prefix/bin/icu-config" --version || true
        echo "Debug: running icu-config --cflags"
        "$icu_prefix/bin/icu-config" --cflags || true
        echo "Debug: running icu-config --ldflags"
        "$icu_prefix/bin/icu-config" --ldflags || true
        return 1
    fi
    ...
    if [ ! -f "$icu_prefix/lib/libicuuc.so.53.2" ] || \
       ...
       [ ! -f "$icu_prefix/bin/icu-config" ]; then
        ...
        ls -la "$icu_prefix/lib/" || echo "Directory not found"
        echo "Files in $icu_prefix/bin:"
        ls -la "$icu_prefix/bin/" || echo "Directory not found"
        return 1
    fi
...
build_php() {
    ...
    if [ "$major" = "5" ] && [ "$PHP_VERSION" = "5.6.40" ]; then
        ...
        if [ ! -f "/usr/local/icu53/bin/icu-config" ]; then
            ...
            echo "   This should have been generated by build_icu53()"
            return 1
        fi
