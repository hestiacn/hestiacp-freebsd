#!/bin/bash
# src/test-php70-openssl4.sh
# TEST: PHP 7.0.33 with OpenSSL 4.0.1
...
scan_openssl_compatibility() {
    ...
    local used_functions_file="$LOG_DIR/php70-used-functions.txt"
    > "$used_functions_file"
...
apply_patches() {
    ...
    if [ -f "ext/dom/dom_iterators.c" ]; then
        sed -i.bak 's/static void itemHashScanner (void \*payload, void \*data, xmlChar \*name)/static void itemHashScanner (void *payload, void *data, const xmlChar *name)/' ext/dom/dom_iterators.c
        echo "[ ✓ ] Patch: dom_iterators.c xmlHashScanner const qualifier added"
    fi
    # ============================================================
    # 补丁4: 替换 OpenSSL 源文件（使用预修改的文件）
    # ============================================================
...
main() {
    echo ""
    echo "========================================"
    echo "TEST: PHP 7.0.33 with OpenSSL 4.0.1"
    echo "========================================"
