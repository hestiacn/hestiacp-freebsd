...
static int php_openssl_make_REQ(struct php_x509_request * req, X509_REQ * csr, zval * dn, zval * attribs)
{
	...
	if (X509_REQ_set_version(csr, 0L)) {
		...
		subj = (X509_NAME *)X509_REQ_get_subject_name(csr);
...
PHP_FUNCTION(openssl_csr_sign)
{
	...
	X509_set_subject_name(new_cert, (X509_NAME *)X509_REQ_get_subject_name(csr));

	if (cert == NULL) {
		cert = new_cert;
	}
	if (!X509_set_issuer_name(new_cert, (X509_NAME *)X509_get_subject_name(cert))) {
		php_openssl_store_errors();
		goto cleanup;
	}
...
PHP_FUNCTION(openssl_csr_get_subject)
{
	...
	subject = (X509_NAME *)X509_REQ_get_subject_name(csr);
