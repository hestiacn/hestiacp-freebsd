...
build_icu72() {
    ...
    if [ -d "$icu_prefix" ] && [ -f "$icu_prefix/lib/libicuuc.so.72.1" ]; then
        echo "[ ✓ ] ICU 72 already installed at $icu_prefix"
        return 0
    fi
    ...
    #fetch -o /tmp/icu-72.tar.gz "https://github.com/unicode-org/icu/archive/refs/tags/release-72-1.tar.gz" || return 1
     echo "[ * ] Copying ICU 72 from local file..."
    LOCAL_ICU_FILE="$SCRIPT_DIR/php8.2/icu-release-72-1.tar.gz"
    cp "$LOCAL_ICU_FILE" /tmp/icu-72.tar.gz || return 1
    tar -xf /tmp/icu-72.tar.gz -C /tmp || return 1
    
    cd /tmp/icu-release-72-1/icu4c/source || return 1
    ...
    echo "[ * ] Building ICU 72 (this may take a while)..."
    mkdir -p ../lib
    
    if ! gmake -j"$NUM_CPUS" | tee /tmp/icu72-build.log; then
        echo "❌ ICU build failed"
        tail -50 /tmp/icu72-build.log
        return 1
    fi
    
    echo "[ * ] Installing ICU 72..."
    if ! gmake install | tee /tmp/icu72-install.log; then
        echo "❌ ICU install failed"
        tail -50 /tmp/icu72-install.log
        return 1
    fi
    echo "[ ✓ ] ICU 72 installation completed successfully"
    
    # 创建符号链接
    echo "[ * ] Creating ICU 72 library symlinks..."
    cd "$icu_prefix/lib"
    for lib in libicuuc libicui18n libicudata libicuio; do
        if [ -f "${lib}.so.72.1" ]; then
            [ ! -f "${lib}.so" ] && ln -sf "${lib}.so.72.1" "${lib}.so"
            [ ! -f "${lib}.so.72" ] && ln -sf "${lib}.so.72.1" "${lib}.so.72"
    ...
    if [ ! -f "$icu_prefix/bin/icu-config" ]; then
        echo "[ * ] Creating icu-config wrapper for ICU 72..."
        cat > "$icu_prefix/bin/icu-config" << 'EOF'
#!/bin/sh
prefix=/usr/local/icu72
exec_prefix=${prefix}
...
EOF
    ...
    # 验证
    echo "[ * ] Verifying ICU 72 installation..."
    if [ -f "$icu_prefix/bin/icu-config" ]; then
        echo "  Version: $($icu_prefix/bin/icu-config --version || echo '72.1')"
    fi
    
    cd /
    rm -rf /tmp/icu-release-72-1 /tmp/icu-72.tar.gz
    
    echo "[ ✓ ] ICU 72 installed successfully"
    return 0
}
...
build_php() {
    ...
    if [ "$major" = "8" ] && [ "$minor" = "2" ]; then
        ...
        if [ ! -f "/usr/local/icu72/bin/icu-config" ]; then
            echo "❌ icu-config not found at /usr/local/icu72/bin/icu-config"
            return 1
        fi
        echo "[ ✓ ] icu-config found"
        
        export PATH="/usr/local/icu72/bin:$PATH"
        
        export CFLAGS="-I/usr/local/icu72/include -I/usr/local/include \
        ...
        export LD_LIBRARY_PATH="/usr/local/lib:/usr/local/icu72/lib:/usr/lib"
        export CXXFLAGS="-std=c++11 -Wno-register -Wno-deprecated-declarations -fpermissive"
        export LDFLAGS="-L/usr/local/icu72/lib -L/usr/local/lib -Wl,-rpath,/usr/local/icu72/lib -Wl,-rpath,/usr/local/lib"
        export CPPFLAGS="-I/usr/local/icu72/include -I/usr/local/include -I/usr/local/include/freetype2"
        export ICU_CONFIG="/usr/local/icu72/bin/icu-config"
        export ICU_PREFIX="/usr/local/icu72"
        export ICU_CFLAGS="-I/usr/local/icu72/include"
        export ICU_LIBS="-L/usr/local/icu72/lib -licui18n -licuuc -licudata -licuio"
    ...
    setup_openssl_env() {
        ...
        # 设置库路径
        export LD_LIBRARY_PATH="/usr/local/icu72/lib:/usr/local/lib:/usr/lib"
...
EOF
    ...
    export ICU_CFLAGS="-I/usr/local/icu72/include"
    export ICU_LIBS="-L/usr/local/icu72/lib -licui18n -licuuc -licudata"
...
EOF
    ...
    CONFIGURE_STATUS=$?
    export LDFLAGS="$LDFLAGS -Wl,-rpath,/usr/local/icu72/lib -Wl,-rpath,/usr/local/lib"
    ...
    if [ -f "Makefile" ]; then
        ...
        echo "[ * ] Forcing ICU 72 library paths..."
        sed -i '' 's|-licuio|/usr/local/icu72/lib/libicuio.so.72.1|g' Makefile
        sed -i '' 's|-licui18n|/usr/local/icu72/lib/libicui18n.so.72.1|g' Makefile
        sed -i '' 's|-licuuc|/usr/local/icu72/lib/libicuuc.so.72.1|g' Makefile
        sed -i '' 's|-licudata|/usr/local/icu72/lib/libicudata.so.72.1|g' Makefile

        if ! grep -q "/usr/local/icu72/lib" Makefile; then
            sed -i '' 's|^LDFLAGS = \(.*\)$|LDFLAGS = -L/usr/local/icu72/lib \1|' Makefile
            sed -i '' 's|^LDFLAGS = \(.*\)$|LDFLAGS = \1 -Wl,-rpath,/usr/local/icu72/lib|' Makefile
        fi
        
        echo "[ ✓ ] Makefile updated to use ICU 72"
    fi
...
create_package() {
    ...
    echo "[ * ] Copying ICU 72 libraries..."
    mkdir -p "${PKG_DIR}/usr/local/icu72/lib"
    if [ -d "/usr/local/icu72/lib" ]; then
        cp -r /usr/local/icu72/lib/libicu*.so* "${PKG_DIR}/usr/local/icu72/lib/" || true
    fi
...
EOF
    ...
    # 设置库路径
    export LD_LIBRARY_PATH="$TEST_ROOT/usr/local/lib:$TEST_ROOT/usr/local/icu72/lib:/usr/local/lib"
