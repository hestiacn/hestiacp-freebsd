#!/bin/bash
# src/test-php74-openssl4.sh
# TEST: PHP 7.4.33 with OpenSSL 4.0.1
...
scan_openssl_compatibility() {
    ...
    # ============================================================
    # 2. 提取 PHP 7.4 openssl.c 中使用的 OpenSSL 函数
    # ============================================================
    echo "[ * ] Extracting PHP 7.4 openssl.c used functions..."
    
    local used_functions_file="$LOG_DIR/php74-used-functions.txt"
    > "$used_functions_file"
    ...
    if [ -f "$available_functions_file" ] && [ -f "$used_functions_file" ]; then
        ...
        echo "Used functions in PHP 7.4: $(wc -l < "$used_functions_file")"
        ...
        
        {
            ...
            if [ "$missing_count" -gt 0 ]; then
                echo "These functions are used by PHP 7.4 but NOT available in OpenSSL 4.0.x:"
                echo ""
...
apply_patches() {
    ...
    head -30 ext/openssl/php_openssl.h
