...
static char *ssl_start_work (SSLSTREAM *stream,char *host,unsigned long flags)
{
  ...
  if (!(flags & NET_NOVALIDATECERT) &&
    ...
    if (scq) return (*scq) (err,host,cert ? X509_NAME_oneline(X509_get_subject_name(cert), NIL, 0) : "???") ? NIL : "";
				/* error message to return via mm_log() */
    sprintf (tmp,"*%.128s: %.255s",err,cert ? X509_NAME_oneline(X509_get_subject_name(cert), NIL, 0) : "???");
...
static char *ssl_validate_cert (X509 *cert,char *host)
{
  ...
  /* locate CN */
  if ((s = strstr(subject_name, "/CN="))) {
    s += 4;  /* skip "/CN=" */
    if ((t = strchr(s, '/'))) *t = '\0';
    
    /* host name matches pattern? */
    ret = ssl_compare_hostnames((unsigned char *)host, (unsigned char *)s) ? NULL :
      "Server name does not match certificate";
    
    if (t) *t = '/';  /* restore smashed delimiter */
    
    /* if mismatch, see if in extensions */
    if (ret && (ext = X509_get_ext_d2i(cert, NID_subject_alt_name, NULL, NULL)) &&
        (n = sk_GENERAL_NAME_num(ext))) {
      for (i = 0; ret && (i < n); i++) {
        if ((name = sk_GENERAL_NAME_value(ext, i)) &&
            (name->type == GEN_DNS) &&
            (s = (char *)ASN1_STRING_get0_data(name->d.ia5)) &&
            ssl_compare_hostnames((unsigned char *)host, (unsigned char *)s)) {
          ret = NULL;
        }
      }
    }
