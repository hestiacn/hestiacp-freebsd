...
#define CRYPTO_set_locking_callback(cb) ((void)0)
...
{
	add_assoc_stringl(val, key, (char *)ASN1_STRING_get0_data(str), ASN1_STRING_length(str));
}
/* }}} */
...
*/
	...
	if (time_len < 13 && time_len != 11) {
		php_error_docref(NULL, E_WARNING, "unable to parse time string %s correctly", time_data);
		return (time_t)-1;
	}
	...
	strbuf = estrdup((const char *)time_data);
...
PHP_MINIT_FUNCTION(openssl)
{
	...
	/* OpenSSL 3.0+ and 4.0+ uses OPENSSL_init_* */
	...
	/* RSA_SSLV23_PADDING was removed in OpenSSL 4.0 */
...
#if OPENSSL_VERSION_NUMBER >= 0x00090805f && OPENSSL_VERSION_NUMBER < 0x30000000L
