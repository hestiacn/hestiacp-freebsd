...
static const SSL_METHOD *php_select_crypto_method(zend_long method_value, int is_client) /* {{{ */
{
	...
	/* OpenSSL 4.0: Use TLS_client_method/TLS_server_method for all */
	return is_client ? (SSL_METHOD *)TLS_client_method() : (SSL_METHOD *)TLS_server_method();
#else
...
#if OPENSSL_VERSION_NUMBER >= 0x40000000L
			/* OpenSSL 4.0: Use TLS_server_method */
...
#if OPENSSL_VERSION_NUMBER >= 0x40000000L
	/* OpenSSL 4.0: Use TLS_client_method/TLS_server_method and set options via flags */
	method = sslsock->is_client ? TLS_client_method() : TLS_server_method();
