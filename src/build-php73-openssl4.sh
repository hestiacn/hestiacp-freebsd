#!/bin/bash
# src/build-php73-openssl4.sh
# Build PHP 7.3.33 with OpenSSL 4.x and create package
...
apply_patches() {
	...
	# 补丁4: reentrancy.c - readdir_r
    if [ -f "main/reentrancy.c" ]; then
        sed -i '' 's/readdir_r(dirp, entry);/readdir_r(dirp, entry, result);/' main/reentrancy.c
        echo "[ ✓ ] Fixed readdir_r in main/reentrancy.c"
    fi

	# 更新版权年份
	if [ -f "./main/main.c" ] && [ -f "./Zend/zend.c" ]; then
		echo "[ * ] Updating copyright year to 2021..."
		find ./main ./Zend ./ext ./sapi ./TSRM -type f \( -name "*.c" -o -name "*.h" \) 2>/dev/null \
			-exec sed -i '' 's/| Copyright (c) The PHP Group.*/| Copyright (c) 1997-2021 The PHP Group                                |/' {} \;
		
		find ./main ./Zend ./ext ./sapi ./TSRM -type f \( -name "*.c" -o -name "*.h" \) 2>/dev/null \
			-exec sed -i '' 's/| Copyright (c) Zend Technologies.*/| Copyright (c) 1998-2021 Zend Technologies Ltd. (http:\/\/www.zend.com) |/' {} \;
		...
		for file in sapi/cli/php_cli.c sapi/fpm/fpm/fpm_main.c sapi/cgi/cgi_main.c sapi/litespeed/lsapi_main.c sapi/phpdbg/phpdbg.c; do
			[ -f "$file" ] && sed -i '' 's/Copyright (c) The PHP Group/Copyright (c) 1997-2021 The PHP Group/g' "$file"
		done
		
		sed -i '' 's/#define ZEND_CORE_VERSION_INFO.*"Zend Engine v" ZEND_VERSION ", Copyright (c) Zend Technologies\\n".*/#define ZEND_CORE_VERSION_INFO\t"Zend Engine v" ZEND_VERSION ", Copyright (c) 1998-2021 Zend Technologies\\n"/' ./Zend/zend.c
		
		echo "[ ✓ ] Copyright updated to 2021"
		grep "Copyright" ./main/main.c 2>/dev/null || true
		grep "Copyright" ./Zend/zend.c 2>/dev/null || true
	fi

	# 补丁5: ext/openssl/php_openssl.h
...
' "ext/openssl/php_openssl.h"
	...
	# ============================================================
	# 补丁6: 替换 OpenSSL 源文件（使用预修改的文件）
	# ============================================================
	local custom_openssl_dir="$SCRIPT_DIR/php7.3"
...
#!/bin/sh
echo "========================================"
echo "PHP 7.3.33 with OpenSSL 4.x installed"
echo "========================================"
