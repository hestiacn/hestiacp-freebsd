#!/bin/bash
# src/build-php85-openssl4.sh
# Build PHP 8.5.8 with OpenSSL 4.x and create package
...
build_php() {
    ...
    if [ -f "/usr/local/lib/libarchive.so" ]; then
        ...
        if [ -L "/usr/local/lib/libarchive.so" ]; then
            ...
            if [ -f "/usr/local/lib/$TARGET" ]; then
                echo "   文件: /usr/local/lib/libarchive.so -> $TARGET"
                echo "   大小: $(stat -f %z "/usr/local/lib/$TARGET") bytes"
                echo "   du -h: $(du -h "/usr/local/lib/$TARGET" | cut -f1)"
                echo "[ * ] Checking OpenSSL dependencies..."
                ldd "/usr/local/lib/$TARGET" | grep -E "(crypto|ssl)"
            fi
        else
    ...
    if [ ! -d "/usr/ports" ]; then
        ...
        if [ -f "/usr/local/lib/libssh2.so" ]; then
            echo "✅ libssh2 编译成功"
            if [ -L "/usr/local/lib/libssh2.so" ]; then
                TARGET=$(readlink /usr/local/lib/libssh2.so)
                if [ -f "/usr/local/lib/$TARGET" ]; then
                    echo "   文件: /usr/local/lib/libssh2.so -> $TARGET"
                    echo "   大小: $(du -h "/usr/local/lib/$TARGET" | cut -f1)"
                    echo "   链接到 OpenSSL:"
                    ldd "/usr/local/lib/$TARGET" | grep -E "(crypto|ssl)" || echo "      ❌ 未链接 OpenSSL"
                fi
            else
                echo "   文件: /usr/local/lib/libssh2.so"
                echo "   大小: $(du -h /usr/local/lib/libssh2.so | cut -f1)"
                ldd /usr/local/lib/libssh2.so | grep -E "(crypto|ssl)" || echo "   ❌ 未链接 OpenSSL"
            fi
        else
            echo "❌ libssh2 编译失败"
            exit 1
        ...
        if [ -f "/usr/local/lib/libcurl.so" ]; then
            ...
            if [ -L "/usr/local/lib/libcurl.so" ]; then
                TARGET=$(readlink /usr/local/lib/libcurl.so)
                if [ -f "/usr/local/lib/$TARGET" ]; then
                    echo "   文件: /usr/local/lib/libcurl.so -> $TARGET"
                    echo "   大小: $(du -h "/usr/local/lib/$TARGET" | cut -f1)"
                    ldd "/usr/local/lib/$TARGET" | grep -E "(crypto|ssl)" || echo "   ❌ 未链接 OpenSSL"
                fi
            else
                ...
                ldd /usr/local/lib/libcurl.so | grep -E "(crypto|ssl)" || echo "   ❌ 未链接 OpenSSL"
            fi
        else
            echo "❌ curl 编译失败"
            exit 1
        ...
        if [ -f "/usr/local/lib/libldap.so" ] || [ -f "/usr/local/lib/libldap.so.2" ]; then
            echo "✅ openldap 编译成功"
            if [ -L "/usr/local/lib/libldap.so" ]; then
                TARGET=$(readlink "/usr/local/lib/libldap.so")
                if [ -f "/usr/local/lib/$TARGET" ]; then
                    echo "   文件: /usr/local/lib/libldap.so -> $TARGET"
                    echo "   大小: $(du -h "/usr/local/lib/$TARGET" | cut -f1)"
                fi
            else
        ...
        if [ -f "/usr/local/lib/libpq.so.5" ] || [ -f "/usr/local/lib/libpq.so" ]; then
            ...
            ls -la /usr/local/lib/libpq.so* | grep -E "libpq\.so" | head -3
        else
        ...
        /* Heimdal compatibility: gss_acquire_cred_with_password is MIT-only */
        #ifdef KRB5_HEIMDAL
        /* Heimdal doesn't have gss_acquire_cred_with_password, but we can use
        krb5_get_init_creds_password + gss_acquire_cred */
        #define gss_acquire_cred_with_password(minor, name, pass, time_req, mechs, cred_usage, cred_handle, actual_mechs, time_rec) \
            gss_acquire_cred(minor, name, time_req, mechs, cred_usage, cred_handle, actual_mechs, time_rec)
        #endif /* KRB5_HEIMDAL */

        EOF

        # 在 gs2.c 中 #include <gssapi/gssapi.h> 后面插入补丁
        sed -i '' '/#include <gssapi\/gssapi.h>/r /tmp/gs2_heimdal.h' gs2.c
        echo "  ✅ Heimdal compatibility patch applied"
        cd ..
        ...
        if [ -f "/usr/local/lib/libsasl2.so" ]; then
            ...
            if [ -L "/usr/local/lib/libsasl2.so" ]; then
                TARGET=$(readlink /usr/local/lib/libsasl2.so)
                if [ -f "/usr/local/lib/$TARGET" ]; then
                    echo "   文件: /usr/local/lib/libsasl2.so -> $TARGET"
                    echo "   大小: $(du -h "/usr/local/lib/$TARGET" | cut -f1)"
                    echo "   链接到 OpenSSL:"
                    ldd "/usr/local/lib/$TARGET" | grep -E "(crypto|ssl)" || echo "      ❌ 未链接 OpenSSL"
                fi
                ...
                ldd /usr/local/lib/libsasl2.so | grep -E "(crypto|ssl)" || echo "   ❌ 未链接 OpenSSL"
            fi
        else
            echo "❌ cyrus-sasl2 编译失败"
            exit 1
        ...
        if grep -q "EVP_RSA_gen" src/osdep/unix/ssl_unix.c 2>/dev/null; then
            ...
            echo "  ❌ src/osdep/unix/ssl_unix.c 不包含 EVP_RSA_gen"
            exit 1
        fi
        ...
        if [ -L "c-client/ssl_unix.c" ]; then
            ...
            echo "  c-client/ssl_unix.c 不是软链接"
        fi

        # 验证内容
        if grep -q "EVP_RSA_gen" c-client/ssl_unix.c 2>/dev/null; then
            echo "  ✅ c-client/ssl_unix.c 包含 EVP_RSA_gen"
        else
            echo "  ❌ c-client/ssl_unix.c 不包含 EVP_RSA_gen"
            exit 1
        fi
...
This is a custom build of PHP 8.5.8 that includes:
