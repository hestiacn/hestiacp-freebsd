...
# Build PHP 8.5.7 with OpenSSL 4.x and create package
...
# ============================================================
# 编译和安装 ICU 78（用于 PHP 8.2）
# ============================================================
build_icu78() {
    local icu_prefix="/usr/local/icu78"
    
    if [ -d "$icu_prefix" ] && [ -f "$icu_prefix/lib/libicuuc.so.78.2" ]; then
        echo "[ ✓ ] ICU 78 already installed at $icu_prefix"
        return 0
    fi
    
    echo "[ * ] Building ICU 78 for PHP 8.2 compatibility..."
    rm -rf "$icu_prefix"
    ...
    tar -xf /tmp/icu-78.tar.gz -C /tmp || return 1
    
    cd /tmp/icu-release-78-2/icu4c/source || return 1
    ...
    echo "[ * ] Building ICU 78 (this may take a while)..."
    mkdir -p ../lib
    
    if ! gmake -j"$NUM_CPUS" | tee /tmp/icu78-build.log; then
        echo "❌ ICU build failed"
        tail -50 /tmp/icu78-build.log
        return 1
    fi
    
    echo "[ * ] Installing ICU 78..."
    if ! gmake install | tee /tmp/icu78-install.log; then
        echo "❌ ICU install failed"
        tail -50 /tmp/icu78-install.log
        return 1
    fi
    echo "[ ✓ ] ICU 78 installation completed successfully"
    
    # 创建符号链接
    echo "[ * ] Creating ICU 78 library symlinks..."
    cd "$icu_prefix/lib"
    for lib in libicuuc libicui18n libicudata libicuio; do
        if [ -f "${lib}.so.78.2" ]; then
            [ ! -f "${lib}.so" ] && ln -sf "${lib}.so.78.2" "${lib}.so"
            [ ! -f "${lib}.so.78" ] && ln -sf "${lib}.so.78.2" "${lib}.so.78"
    ...
    if [ ! -f "$icu_prefix/bin/icu-config" ]; then
        echo "[ * ] Creating icu-config wrapper for ICU 78..."
        cat > "$icu_prefix/bin/icu-config" << 'EOF'
#!/bin/sh
prefix=/usr/local/icu78
exec_prefix=${prefix}
...
EOF
    ...
    # 验证
    echo "[ * ] Verifying ICU 78 installation..."
    if [ -f "$icu_prefix/bin/icu-config" ]; then
        echo "  Version: $($icu_prefix/bin/icu-config --version || echo '78.2')"
    fi
    
    cd /
    rm -rf /tmp/icu-release-78-2 /tmp/icu-78.tar.gz
    
    echo "[ ✓ ] ICU 78 installed successfully"
    return 0
}
...
build_php() {
    ...
    # ============================================================
    # PHP 8.2 特殊处理：使用 ICU 78
    # ============================================================
    if [ "$major" = "8" ] && [ "$minor" = "2" ]; then
        # 编译 ICU 78
        if ! build_icu78; then
            echo "❌ Failed to build ICU 78"
            return 1
        fi
        ...
        if [ ! -f "/usr/local/icu78/bin/icu-config" ]; then
            echo "❌ icu-config not found at /usr/local/icu78/bin/icu-config"
            return 1
        fi
        echo "[ ✓ ] icu-config found"
        
        export PATH="/usr/local/icu78/bin:$PATH"
        
        export CFLAGS="-I/usr/local/icu78/include -I/usr/local/include \
        ...
        export LD_LIBRARY_PATH="/usr/local/lib:/usr/local/icu78/lib:/usr/lib"
        export CXXFLAGS="-std=c++11 -Wno-register -Wno-deprecated-declarations -fpermissive"
        export LDFLAGS="-L/usr/local/icu78/lib -L/usr/local/lib -Wl,-rpath,/usr/local/icu78/lib -Wl,-rpath,/usr/local/lib"
        export CPPFLAGS="-I/usr/local/icu78/include -I/usr/local/include -I/usr/local/include/freetype2"
        export ICU_CONFIG="/usr/local/icu78/bin/icu-config"
        export ICU_PREFIX="/usr/local/icu78"
        export ICU_CFLAGS="-I/usr/local/icu78/include"
        export ICU_LIBS="-L/usr/local/icu78/lib -licui18n -licuuc -licudata -licuio"
        ...
        echo "[ ✓ ] ICU config version: $(icu-config --version || echo '78.2')"
    fi
    ...
    setup_openssl_env() {
        ...
        # 设置库路径
        export LD_LIBRARY_PATH="/usr/local/icu78/lib:/usr/local/lib:/usr/lib"
...
EOF
    ...
    export ICU_CFLAGS="-I/usr/local/icu78/include"
    export ICU_LIBS="-L/usr/local/icu78/lib -licui18n -licuuc -licudata"
	...
	echo "OpenSSL prefix: ${OPENSSL_PREFIX:-/usr/local}"
	echo "CFLAGS: $CFLAGS"
    export LDFLAGS="-L/usr/local/icu78/lib -L/usr/local/lib ${LDFLAGS}"
...
EOF
    ...
    CONFIGURE_STATUS=$?
    export LDFLAGS="$LDFLAGS -Wl,-rpath,/usr/local/icu78/lib -Wl,-rpath,/usr/local/lib"
    ...
    if [ -f "Makefile" ]; then
        ...
        echo "[ * ] Forcing ICU 78 library paths..."
        sed -i '' 's|-licuio|/usr/local/icu78/lib/libicuio.so.78.2|g' Makefile
        sed -i '' 's|-licui18n|/usr/local/icu78/lib/libicui18n.so.78.2|g' Makefile
        sed -i '' 's|-licuuc|/usr/local/icu78/lib/libicuuc.so.78.2|g' Makefile
        sed -i '' 's|-licudata|/usr/local/icu78/lib/libicudata.so.78.2|g' Makefile

        if ! grep -q "/usr/local/icu78/lib" Makefile; then
            sed -i '' 's|^LDFLAGS = \(.*\)$|LDFLAGS = -L/usr/local/icu78/lib \1|' Makefile
            sed -i '' 's|^LDFLAGS = \(.*\)$|LDFLAGS = \1 -Wl,-rpath,/usr/local/icu78/lib|' Makefile
        fi
        
        echo "[ ✓ ] Makefile updated to use ICU 78"
    fi
...
create_package() {
    ...
    echo "[ * ] Copying ICU 78 libraries..."
    mkdir -p "${PKG_DIR}/usr/local/icu78/lib"
    if [ -d "/usr/local/icu78/lib" ]; then
        cp -r /usr/local/icu78/lib/libicu*.so* "${PKG_DIR}/usr/local/icu78/lib/" || true
    fi
...
EOF
    ...
    # 设置库路径
    export LD_LIBRARY_PATH="$TEST_ROOT/usr/local/lib:$TEST_ROOT/usr/local/icu78/lib:/usr/local/lib"
