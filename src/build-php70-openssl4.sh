...
build_icu56() {
    ...
    #fetch -o /tmp/icu-56.tar.gz "https://github.com/unicode-org/icu/archive/refs/tags/release-56-2.tar.gz" || return 1
    echo "[ * ] Copying ICU 56 from local file..."
    
    LOCAL_ICU_FILE="$SCRIPT_DIR/php7.0/icu-release-56-2.tar.gz"
    
    cp "$LOCAL_ICU_FILE" /tmp/icu-56.tar.gz || return 1
    ...
    if ! gmake -j"$NUM_CPUS" | tee /tmp/icu56-build.log; then
    ...
    if ! gmake install | tee /tmp/icu56-install.log; then
        echo "❌ ICU install failed"
        tail -50 /tmp/icu56-install.log
        return 1
    fi
    echo "[ ✓ ] ICU 56 installation completed successfully"
...
apply_patches() {
    ...
    # 补丁7: 修复 main/reentrancy.c 中的 readdir_r (FreeBSD 14) - 强制走 readdir
    if [ -f "main/reentrancy.c" ]; then
        echo "[ * ] Patching main/reentrancy.c to use readdir() instead of readdir_r..."
        
        echo "  Before patch:"
        sed -n '128,155p' main/reentrancy.c
        echo ""
        
        # 禁用 HAVE_OLD_READDIR_R 分支，强制走 readdir + 锁
        sed -i '' 's/#if defined(HAVE_OLD_READDIR_R)/#if 0 \/* HAVE_OLD_READDIR_R disabled, using readdir() *\//' main/reentrancy.c
        
        echo "  After patch:"
        sed -n '128,155p' main/reentrancy.c
        echo ""
        
        echo "  ✅ Using readdir() with locking instead of readdir_r"
    fi

    # 补丁8: 修复 main/reentrancy.c 中的 readdir_r (FreeBSD 14)
    if [ -f "main/reentrancy.c" ]; then
    ...
    # 补丁9: 修复 main/streams/cast.c 中的函数指针类型 (FreeBSD 14)
    if [ -f "main/streams/cast.c" ]; then
        echo "[ * ] Patching main/streams/cast.c for function pointer type..."
        # 修改第118行的函数签名: zend_off_t position -> off64_t *position
    ...
    # ============================================================
    # 补丁11: 完整修复 phar 扩展 OpenSSL 4.x 兼容性
    # ============================================================
...
EOF
    ...
    # 2. 修复 phar.c（OpenSSL 调用 + 目录遍历）
    if [ -f "ext/phar/phar.c" ]; then
        cp ext/phar/phar.c ext/phar/phar.c.bak
        
        echo "  Before patch (OpenSSL calls):"
        grep -n "SSL_library_init\|EVP_cleanup\|SSL_get_peer_certificate" ext/phar/phar.c | head -5 || echo "  (pattern not found)"
        
        echo ""
        echo "  Before patch (readdir section):"
        grep -n -B2 -A2 "entry = readdir" ext/phar/phar.c | head -20 || echo "  (pattern not found)"
        
        # 修复 OpenSSL 调用
        perl -pi -e '
            s/SSL_library_init\(\);/PHAR_OPENSSL_INIT();/g;
            s/OpenSSL_add_all_ciphers\(\);/PHAR_OPENSSL_INIT();/g;
            s/OpenSSL_add_all_digests\(\);/PHAR_OPENSSL_INIT();/g;
            s/EVP_cleanup\(\);/PHAR_OPENSSL_CLEANUP();/g;
            s/SSL_get_peer_certificate\(/SSL_get1_peer_certificate(/g;
        ' ext/phar/phar.c
        
        # 修复目录遍历
        perl -pi -e '
            s/struct dirent \*entry;/struct dirent *entry, entry_buf;/g;
            s/entry = readdir\(dir\)/readdir_r(dir, \&entry_buf, \&entry)/g;
            s/if \(entry\.filename_len == 0\) {/if (entry.filename_len == 0 || buffer == NULL) {/g;
            s/phar_add_virtual_dirs\(mydata, buffer, entry\.filename_len\);/if (buffer \&\& entry.filename_len > 0) { phar_add_virtual_dirs(mydata, buffer, entry.filename_len); }/g;
            s/entry\.filename = pestrndup\(buffer, entry\.filename_len, entry\.is_persistent\);/if (buffer \&\& entry.filename_len > 0) { entry.filename = pestrndup(buffer, entry.filename_len, entry.is_persistent); } else { continue; }/g;
        ' ext/phar/phar.c
        
        echo ""
        echo "  After patch (OpenSSL calls):"
        grep -n "PHAR_OPENSSL_INIT\|PHAR_OPENSSL_CLEANUP\|SSL_get1_peer_certificate" ext/phar/phar.c | head -5 || echo "  (pattern not found)"
        
        echo ""
        echo "  After patch (readdir section):"
        grep -n -B2 -A2 "readdir_r" ext/phar/phar.c | head -20 || echo "  (pattern not found)"
        
        echo ""
        echo "  Diff (phar.c):"
        diff -u ext/phar/phar.c.bak ext/phar/phar.c | head -60 || echo "  No changes"
        
        # 验证
        echo ""
        echo "  Verification:"
        if grep -q "PHAR_OPENSSL_INIT" ext/phar/phar.c; then
            echo "    ✅ PHAR_OPENSSL_INIT found"
        else
            echo "    ❌ PHAR_OPENSSL_INIT NOT found"
        fi
        if grep -q "readdir_r" ext/phar/phar.c; then
            echo "    ✅ readdir_r found"
        else
            echo "    ❌ readdir_r NOT found"
        fi
        if grep -q "buffer == NULL" ext/phar/phar.c; then
            echo "    ✅ NULL check found"
        else
            echo "    ❌ NULL check NOT found"
        fi
    ...
    # 2. 修复 dirstream.c
    echo "[ * ] Fixing phar dirstream for OpenSSL 4.x..."
    
    if [ -f "ext/phar/dirstream.c" ]; then
        cp ext/phar/dirstream.c ext/phar/dirstream.c.bak
        
        echo "  Before patch (dirstream.c - readdir loop):"
        grep -n -B2 -A2 "php_stream_readdir" ext/phar/dirstream.c | head -20
        
        # 修复1: 初始化 entry 并添加空指针检查
        perl -pi -e '
            s/php_stream_dirent entry;/php_stream_dirent entry; memset(\&entry, 0, sizeof(entry));/g;
            s/while \(php_stream_readdir\(dir, \&entry\)\)/while (php_stream_readdir(dir, \&entry) \&\& entry.d_name != NULL)/g;
        ' ext/phar/dirstream.c
        
        echo ""
        echo "  After patch (dirstream.c - readdir loop):"
        grep -n -B2 -A2 "php_stream_readdir" ext/phar/dirstream.c | head -20
        
        echo ""
        echo "  Before patch (phar_make_dirstream - hash table traversal):"
        sed -n '180,220p' ext/phar/dirstream.c
        
        # 修复2: 在遍历前创建 manifest 副本
        perl -pi -e '
            # 添加 manifest_copy 变量
            s/HashTable \*data;/HashTable *data;\n\tHashTable manifest_copy;/g;
            
            # 在 ALLOC_HASHTABLE 之后复制 manifest
            s/ALLOC_HASHTABLE\(data\);/ALLOC_HASHTABLE(data);\n\tzend_hash_init(&manifest_copy, 64, NULL, NULL, 0);\n\tzend_hash_copy(&manifest_copy, manifest, NULL);\n\tmanifest = &manifest_copy;/g;
            
            # 在函数返回前销毁副本
            s/return php_stream_alloc\(&phar_dir_ops, data, NULL, "r"\);/zend_hash_destroy(manifest);\n\treturn php_stream_alloc(&phar_dir_ops, data, NULL, "r");/g;
        ' ext/phar/dirstream.c
        
        echo ""
        echo "  After patch (phar_make_dirstream - hash table traversal):"
        sed -n '180,220p' ext/phar/dirstream.c
        
        echo ""
        echo "  Diff (dirstream.c):"
        diff -u ext/phar/dirstream.c.bak ext/phar/dirstream.c | head -50 || echo "  No changes"
        
        echo "  ✓ Fixed phar dirstream"
    fi
    
    # 3. 修复 phar_object.c
    if [ -f "ext/phar/phar_object.c" ]; then
        cp ext/phar/phar_object.c ext/phar/phar_object.c.bak
        
        echo ""
        echo "  Before patch (phar_object.c - directory check):"
        grep -n -B2 -A2 "if (!dir)" ext/phar/phar_object.c | head -15
        
        perl -pi -e '
            s/if \(!dir\)/if (!dir || !dir->abstract)/g;
            s/php_stream_dirent entry;/php_stream_dirent entry; memset(\&entry, 0, sizeof(entry));/g;
        ' ext/phar/phar_object.c
        
        echo ""
        echo "  After patch (phar_object.c - directory check):"
        grep -n -B2 -A2 "if (!dir" ext/phar/phar_object.c | head -15
        
        echo ""
        echo "  Diff (phar_object.c):"
        diff -u ext/phar/phar_object.c.bak ext/phar/phar_object.c | head -30 || echo "  No changes"
        
        echo "  ✓ Fixed phar_object.c"
    fi
    
    echo ""
    echo "  ✓ All phar OpenSSL 4.x fixes applied"

    # 补丁12: 修复 ext/zip/lib/mkstemp.c 缺少 getpid 声明
    if [ -f "ext/zip/lib/mkstemp.c" ]; then
        if ! grep -q "#include <unistd.h>" ext/zip/lib/mkstemp.c; then
            echo "  Before (mkstemp.c) - around line 28-40:"
            sed -n '28,40p' ext/zip/lib/mkstemp.c
            echo ""
            perl -pi -e 's/#include <fcntl.h>/#include <fcntl.h>\n#include <unistd.h>/' ext/zip/lib/mkstemp.c
            echo "  After (mkstemp.c) - around line 28-41:"
            sed -n '28,41p' ext/zip/lib/mkstemp.c
            echo ""
            
            echo "  ✓ Added #include <unistd.h> to mkstemp.c (after fcntl.h)"
        else
            echo "  ✓ mkstemp.c already has unistd.h"
        fi
    fi
...
build_php() {
	...
	if [ "$major" = "7" ] && [ "$PHP_VERSION" = "7.0.33" ]; then
        ...
        export LD_LIBRARY_PATH="/usr/local/lib:/usr/local/icu56/lib:/usr/lib"
...
EOF
    ...
    echo "OpenSSL prefix: ${OPENSSL_PREFIX:-/usr/local}"
    echo "CFLAGS: $CFLAGS"
    export LDFLAGS="-L/usr/local/icu56/lib -L/usr/local/lib ${LDFLAGS}"
    ...
    CONFIGURE_STATUS=$?
    export LDFLAGS="$LDFLAGS -Wl,-rpath,/usr/local/icu56/lib -Wl,-rpath,/usr/local/lib"
    ...
    if [ -f "$install_dir/usr/local/bin/php" ]; then
        ...
        echo "[ DEBUG ] build_php() completed successfully, returning 0"
        return 0
    else
        echo "❌ PHP binary not found!"
        echo "[ DEBUG ] Contents of $install_dir/usr/local/bin:"
        ls -la "$install_dir/usr/local/bin/" || echo "Directory does not exist"
        return 1
    fi
}
...
EOF
	...
	# 获取扩展目录
	EXT_DIR="$TEST_ROOT/usr/local/lib/php/extensions/no-debug-non-zts-20151012"
...
EOF
	...
	# 设置库路径
	export LD_LIBRARY_PATH="$TEST_ROOT/usr/local/lib:$TEST_ROOT/usr/local/icu56/lib:/usr/local/lib"
	...
	pkg info -l "${PKG_FILE}" || tar -tf "${PKG_FILE}" || {
		echo "⚠️  Cannot list package contents"
		echo "Files in ${PKG_DIR}:"
