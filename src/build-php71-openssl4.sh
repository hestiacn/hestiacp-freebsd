#!/bin/bash
# src/build-php71-openssl4.sh
# Build PHP 7.1.33 with OpenSSL 4.x and create package
...
apply_patches() {
	...
	# 补丁7: 替换 OpenSSL 源文件
	# ============================================================
	local custom_openssl_dir="$SCRIPT_DIR/php7.1"
    cp "$custom_openssl_dir/openssl.c" "ext/openssl/openssl.c"
    cp "$custom_openssl_dir/php_openssl.h" "ext/openssl/php_openssl.h"
    cp "$custom_openssl_dir/xp_ssl.c" "ext/openssl/xp_ssl.c"

    # ============================================================
    # 补丁8: 修复 gd.c 中的 FreeType 头文件路径
    # ============================================================
    if [ -f "ext/gd/gd.c" ]; then
        echo "[ * ] Patching gd.c to use correct FreeType path..."
        if grep -q '# include <ft2build.h>' ext/gd/gd.c && ! grep -q '# include <freetype2/ft2build.h>' ext/gd/gd.c; then
            sed -i '' 's|# include <ft2build.h>|# include <freetype2/ft2build.h>|' ext/gd/gd.c
            echo "  ✅ Patched gd.c: <ft2build.h> -> <freetype2/ft2build.h>"
        fi
    fi

    # ============================================================
    # 补丁9: 修复 ext/zip/lib/mkstemp.c 缺少 getpid 声明
    # ============================================================
    if [ -f "ext/zip/lib/mkstemp.c" ]; then
        echo "[ * ] Patching ext/zip/lib/mkstemp.c for getpid()..."
        if ! grep -q "#include <unistd.h>" ext/zip/lib/mkstemp.c; then
            sed -i '' '1i\
#include <unistd.h>
' ext/zip/lib/mkstemp.c
            echo "  ✅ Added #include <unistd.h> to mkstemp.c"
        else
            echo "  ✅ mkstemp.c already has unistd.h"
        fi
    fi

    # 补丁10: 修复 main/reentrancy.c 中的 readdir_r (FreeBSD 14)
    ...
    # 补丁11: 修复 main/streams/cast.c 中的函数指针类型
    if [ -f "main/streams/cast.c" ]; then
        echo "[ * ] Patching main/streams/cast.c for function pointer type..."
        echo "  Before patch (int version, line ~118):"
        sed -n '115,125p' main/streams/cast.c
        echo ""
...
build_php() {
	...
	# ============================================================
	if [ "$major" = "7" ] && [ "$PHP_VERSION" = "7.1.33" ]; then
		# 编译 ICU 57
		if ! build_icu57; then
...
EOF
    ...
    # ============================================================
    # 创建 freetype-config 包装器（PHP configure 需要）
    # ============================================================
    echo "[ * ] Creating freetype-config wrapper..."
...
EOF
    ...
    # 验证
    if /usr/local/bin/freetype-config --version >/dev/null 2>&1; then
        echo "  ✅ freetype-config ready: $(/usr/local/bin/freetype-config --version)"
    else
        echo "  ❌ freetype-config not working!"
        exit 1
    fi

    # ============================================================
    # 修复 readdir_r 检测（FreeBSD 14 兼容）
    # ============================================================
    echo "[ * ] Fixing readdir_r detection for FreeBSD 14..."
    export ac_cv_func_readdir_r=yes
    export ac_cv_func_readdir_r_posix=yes
    export ac_cv_func_old_readdir_r=no
    echo "  ✅ readdir_r forced to POSIX version"
...
EOF
    ...
    ./configure \
        ...
        ac_cv_func_readdir_r_posix=yes \
        ac_cv_func_old_readdir_r=no \
...
EOF
    ...
    echo ""
    PKG_NAME="php${ver_suffix}-openssl4-icu57"
    ...
    echo "[ DEBUG ] PKG_DIR after cleanup: ${PKG_DIR}"
...
This is a custom build of PHP 7.1.33 that includes:
...
IMPORTANT: PHP 7.1 is end-of-life. Use at your own risk.
EOD
EOF
...
EOF
    ...
    if tar -xf "$PKG_FILE" -C "$TEST_ROOT" 2>/dev/null; then
...
EOF
        ...
        if pkg -c "$TEST_ROOT" add "$PKG_FILE" 2>&1; then
            ...
            if bsdtar -xf "$PKG_FILE" -C "$TEST_ROOT" 2>/dev/null; then
